...
class KafkaMonitor:
    ...
    def _process_messages(self):
        try:
            for message in self.consumer.get_messages():
                self._process_message(message.message.value)
    ...
    def _process_message(self, message_json):
        try:
            ...
            self._get_and_process_plugins(message_dict)
        except ValueError:
            extras = {}
            extras['parsed'] = False
            extras['valid'] = False
    ...
    def _get_and_process_plugins(self, message_dict):
        try:
            for key in self.plugins_dict:
